...
/*
	Scan num lines at a time
*/

func ScanLines(scanner *bufio.Scanner, num int) (linea []string, err error) {
...
func ScanAllLines(scanner *bufio.Scanner) (linea []string, err error) {
	stop := false
	for !stop {
		scanner.Scan()
		eof := false
...
func WriteFile(filename string, buf []byte, mode os.FileMode) error {
	...
	if err = ioutil.WriteFile(filename, buf, mode); err != nil {
		fmt.Printf("Err %v Writing %s", err, filename)
	}
	return err
}
